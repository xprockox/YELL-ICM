### Integrated Community Model (ICM) - Stage 1: Parameter estimations
...
elk_abund_stage2 <- elk_N_summ %>%
  ...
  filter(stage %in% c("yearling", "ya", "oa", "female")) %>%
  pivot_wider(
    id_cols = year,
    names_from = stage,
    values_from = c(mean, low, high),
    names_glue = "elk_N_{stage}_{.value}"
  ) %>%
  rename(
    elk_N_yearling = elk_N_yearling_mean,
    elk_N_yearling_low = elk_N_yearling_low,
    elk_N_yearling_high = elk_N_yearling_high,
    
    elk_N_ya = elk_N_ya_mean,
    elk_N_ya_low = elk_N_ya_low,
    elk_N_ya_high = elk_N_ya_high,
    
    elk_N_oa = elk_N_oa_mean,
    elk_N_oa_low = elk_N_oa_low,
    elk_N_oa_high = elk_N_oa_high,
    
    elk_N_female = elk_N_female_mean,
    elk_N_female_low = elk_N_female_low,
    elk_N_female_high = elk_N_female_high
  )

# wolf abundance
wolf_abund_stage2 <- wolf_N_summ %>%
  mutate(
    stage = recode(
      stage,
      "Pups" = "p",
  ...
  filter(stage %in% c("p", "a", "tot")) %>%
  pivot_wider(
    id_cols = year,
    names_from = stage,
    values_from = c(mean, low, high),
    names_glue = "wolf_N_{stage}_{.value}"
  ) %>%
  rename(
    wolf_N_p = wolf_N_p_mean,
    wolf_N_p_low = wolf_N_p_low,
    wolf_N_p_high = wolf_N_p_high,
    
    wolf_N_a = wolf_N_a_mean,
    wolf_N_a_low = wolf_N_a_low,
    wolf_N_a_high = wolf_N_a_high,
    
    wolf_N_tot = wolf_N_tot_mean,
    wolf_N_tot_low = wolf_N_tot_low,
    wolf_N_tot_high = wolf_N_tot_high
  )

# elk vital rates
elk_rates_stage2 <- elk_vrates2 %>%
  filter(rate %in% c(
    "Calf survival (s_c)",
    "Young Adult survival (s_ya)",
    "Old Adult survival (s_oa)",
  ...
  select(year, rate, mean, low, high) %>%
  pivot_wider(
    names_from = rate,
    values_from = c(mean, low, high)
  )
...
# wolf vital rates
wolf_rates_stage2 <- wolf_vrates2 %>%
  filter(rate %in% c(
    "Pup survival (s_p)",
    "Adult survival (s_a)",
    "Fecundity (f)"
...
# combine everything
stage2_df <- elk_rates_stage2 %>%
  left_join(wolf_rates_stage2, by = "year") %>%
  left_join(elk_abund_stage2, by = "year") %>%
  left_join(wolf_abund_stage2, by = "year") %>%
  arrange(year)

stop('WARNING: The following lines will overwrite data. Are you sure you would like to proceed?')
write.csv(stage2_df, 'data/outputs/wolf_elk_ICM_estimates_2026Apr13.csv')
